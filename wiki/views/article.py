...
from wiki.functions.markdown import editors
...
from wiki.functions.paginator import WikiPaginator
from wiki.functions import registry as plugin_registry
from wiki.functions.utils import object_to_json_response
from wiki.decorators import get_article
from wiki.functions.mixins import ArticleMixin
...
class Edit(ArticleMixin, FormView):
    ...
    def get_form(self, form_class=None):
        """
        从查询字符串数据中检查编辑表单是否正在实际保存，否则将从表单初始化中删除“data”和“files”kwargs。
        """
        if form_class is None:
            form_class = self.get_form_class()
        kwargs = self.get_form_kwargs()
    ...
    def get_sidebar_form_classes(self):
        """
        返回侧边栏的表单类字典。如果没有表单类指定，将None放入字典中。使用字典中的键以标识正在保存的表单。
        """
        form_classes = {}
        ...
        if form.cleaned_data["redirect"]:
            ...
            for descendant in descendants:
                ...
                urlpath_new = models.URLPath._create_urlpath_from_request(
                    ...
                    parent_urlpath,
                    slug,
                    ("Moved: {title}").format(title=descendant.article),
                    ("Article moved to {link}").format(link=link),
                    ("Created redirect (auto)"),
                )
            ...
            messages.success(self.request, ("文章已成功移动！"))
        return redirect("wiki:get", path=self.urlpath.path)


class Deleted(Delete):
    """
    告诉用户文章已被删除。如果用户具有权限，让用户还原并可能清除已删除的文章和子项。
    """
    ...
    def dispatch(self, request, article, *args, **kwargs):
        ...
        if request.GET.get("restore", False):
            ...
            if can_restore:
                ...
                revision.deleted = False
                revision.automatic_log = ("Restoring article")
                self.article.add_revision(revision)
                messages.success(
                    request,
                    ('现在已还原项目“%s”及其子项。')
...
class ChangeRevisionView(RedirectView):
    ...
    def change_revision(self):
        ...
        messages.success(
            self.request,
            (
                "文章%(title)s现在设置为显示修订#%(revision_number)d"
            )
...
class DiffView(DetailView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        if not other_revision or other_revision.title != revision.title:
            other_changes.append((("New title"), revision.title))
